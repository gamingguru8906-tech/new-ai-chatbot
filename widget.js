(function () {
  ...
  function injectStyles() {
    ...
    // Inject Google Fonts (Cinzel for brand/headings, Fraunces for chat body, IBM Plex Sans for UI labels)
    if (!document.getElementById("maya-credits-font")) {
      var font = document.createElement("link");
      font.id = "maya-credits-font";
      font.rel = "stylesheet";
      font.href = "https://fonts.googleapis.com/css2?family=Cinzel:wght@500;600&family=Fraunces:opsz,wght@9..144,400;9..144,500&family=IBM+Plex+Sans:wght@400;500;600&display=swap";
      document.head.appendChild(font);
    }
    var style = document.createElement("style");
    style.id = "maya-credits-widget-style";
    style.textContent = [
      // Launcher — navy pill, Cinzel lettering, beige text
      ".maya-launcher{position:fixed;right:22px;bottom:22px;z-index:2147482500;border:0;border-radius:999px;padding:13px 22px;background:#1E2A42;color:#EDE3CF;box-shadow:0 18px 44px rgba(30,42,66,.32);font-family:'Cinzel',serif;font-weight:600;font-size:13px;letter-spacing:1.5px;cursor:pointer;transition:background .18s ease,box-shadow .18s ease}.maya-launcher:hover{background:#34456B;box-shadow:0 22px 50px rgba(30,42,66,.38)}",
      // Overlay — warm dark scrim
      ".maya-overlay{position:fixed;inset:0;z-index:2147482501;display:none;align-items:center;justify-content:center;padding:22px;background:rgba(22,14,8,.55);backdrop-filter:blur(14px)}.maya-overlay.is-open{display:flex}",
      // Box — warm beige, brown hairline border
      ".maya-box{width:min(760px,calc(100vw - 30px));max-height:calc(100svh - 34px);overflow:auto;background:#EDE3CF;color:#1E2A42;border:1px solid #A98F66;border-radius:14px;box-shadow:0 32px 100px rgba(22,14,8,.38)}",
      // Head — slightly deeper beige, brown separator
      ".maya-head{min-height:88px;padding:18px 22px;display:flex;align-items:flex-start;justify-content:space-between;gap:18px;background:#DCC9A3;border-bottom:1px solid #A98F66}",
      // Brand — orb becomes an instrument-dial roundel (CSS-only, no SVG injection needed)
      ".maya-brand{display:flex;gap:14px;align-items:center}.maya-orb{width:50px;height:50px;border-radius:50%;background:#EDE3CF;border:2px solid #1E2A42;box-shadow:0 4px 18px rgba(30,42,66,.18);position:relative;flex-shrink:0}.maya-orb::after{content:'\\2726';position:absolute;inset:0;display:grid;place-items:center;font-size:17px;color:#1E2A42;line-height:1}.maya-brand h3{margin:0;color:#1E2A42;font-family:'Cinzel',serif;font-weight:600;font-size:19px;letter-spacing:2px}.maya-brand p{margin:4px 0 0;color:#6B6357;font-family:'IBM Plex Sans',sans-serif;font-size:12px;letter-spacing:.06em}",
      // Close — ghost button, navy icon
      ".maya-close{width:44px;height:44px;border:1px solid #A98F66;border-radius:10px;background:transparent;color:#1E2A42;font-size:22px;font-weight:600;cursor:pointer;transition:background .15s ease,border-color .15s ease}.maya-close:hover{background:#CDBA98;border-color:#6B6357}",
      // Content text — Cinzel title, IBM Plex for kicker and copy
      ".maya-content{padding:22px}.maya-kicker{color:#34456B;font-family:'IBM Plex Sans',sans-serif;font-size:11px;font-weight:700;letter-spacing:.18em;text-transform:uppercase}.maya-title{margin:7px 0 10px;color:#1E2A42;font-family:'Cinzel',serif;font-weight:600;font-size:clamp(22px,3vw,32px);line-height:1.1}.maya-copy{margin:0 0 18px;color:#6B6357;font-family:'IBM Plex Sans',sans-serif;font-size:14px;line-height:1.55}",
      // Choice cards — lifted beige surface, navy Cinzel strong, gray desc
      ".maya-choice-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.maya-choice-card{min-height:176px;padding:19px;display:grid;align-content:start;gap:10px;background:#F7F1E4;color:#1E2A42;border:1px solid #CDBA98;border-radius:12px;box-shadow:0 8px 28px rgba(30,42,66,.08);text-align:left;cursor:pointer}.maya-choice-card strong{font-family:'Cinzel',serif;font-weight:600;font-size:16px;line-height:1.2;color:#1E2A42}.maya-choice-card span{color:#6B6357;font-family:'IBM Plex Sans',sans-serif;font-size:13px;line-height:1.45}",
      // Panel — same beige system, navy border
      ".maya-panel{position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);z-index:2147482502;width:min(720px,calc(100vw - 30px));height:min(760px,calc(100svh - 34px));display:none;grid-template-rows:auto minmax(0,1fr) auto;background:#EDE3CF;border:1px solid #A98F66;border-radius:14px;box-shadow:0 32px 100px rgba(22,14,8,.38);overflow:hidden}.maya-panel.is-open{display:grid}",
      // Body / foot / input / buttons
      ".maya-body{padding:18px;overflow:auto;background:linear-gradient(180deg,#F7F1E4 0%,#EDE3CF 100%)}.maya-foot{display:flex;gap:10px;padding:14px;border-top:1px solid #CDBA98;background:#EDE3CF}.maya-input{flex:1;min-height:48px;padding:0 14px;background:#F7F1E4;color:#1E2A42;border:1px solid #CDBA98;border-radius:10px;font-family:'Fraunces',serif;font-size:15px;outline:none;transition:border-color .15s ease}.maya-input:focus{border-color:#34456B}.maya-send,.maya-primary{min-height:46px;padding:0 20px;border:0;border-radius:999px;background:#1E2A42;color:#EDE3CF;font-family:'IBM Plex Sans',sans-serif;font-weight:600;font-size:13px;letter-spacing:.4px;cursor:pointer;transition:background .15s ease}.maya-send:hover,.maya-primary:hover{background:#34456B}.maya-secondary{min-height:46px;padding:0 16px;border:1px solid #A98F66;border-radius:999px;background:#DCC9A3;color:#1E2A42;font-family:'IBM Plex Sans',sans-serif;font-weight:600;font-size:13px;cursor:pointer;transition:background .15s ease}.maya-secondary:hover{background:#CDBA98}",
      // Chat messages — Fraunces for Maya, navy bg for user
      ".maya-msg{max-width:82%;margin:0 0 12px;padding:12px 15px;border-radius:12px;font-family:'Fraunces',serif;font-size:15px;line-height:1.6}.maya-bot{background:#F7F1E4;color:#1E2A42;border:1px solid #CDBA98}.maya-user{margin-left:auto;background:#1E2A42;color:#EDE3CF}",
      // Credit bar and upgrade prompt
      ".maya-credit-bar{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:0 0 14px;padding:10px 14px;background:#DCC9A3;border:1px solid #CDBA98;border-radius:10px;color:#1E2A42;font-family:'IBM Plex Sans',sans-serif;font-size:13px;font-weight:600}.maya-credit-bar span{color:#34456B;font-weight:700}.maya-upgrade{margin:12px 0 0;padding:14px;background:#F7F1E4;border:1px solid #CDBA98;border-radius:12px;color:#1E2A42}.maya-upgrade p{margin:0 0 10px;color:#6B6357;font-family:'IBM Plex Sans',sans-serif;font-size:13.5px;line-height:1.55}.maya-pay{min-height:44px;padding:0 18px;border:0;border-radius:999px;background:#34456B;color:#EDE3CF;font-family:'IBM Plex Sans',sans-serif;font-weight:600;font-size:13px;cursor:pointer;transition:background .15s ease}.maya-pay:hover{background:#1E2A42}",
      // Progress bar — navy fill on beige track
      ".maya-progress{height:6px;margin:12px 0 18px;overflow:hidden;background:#CDBA98;border-radius:999px}.maya-progress span{display:block;height:100%;background:linear-gradient(90deg,#1E2A42,#34456B);border-radius:999px}",
      // Form fields
      ".maya-field{display:grid;gap:8px}.maya-field label{color:#34456B;font-family:'IBM Plex Sans',sans-serif;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.1em}.maya-field input{min-height:52px;padding:0 14px;background:#F7F1E4;color:#1E2A42;border:1px solid #CDBA98;border-radius:10px;font-family:'IBM Plex Sans',sans-serif;font-size:16px;outline:none;transition:border-color .15s ease}.maya-field input:focus{border-color:#34456B}",
      // Field wrap, icon button, hint, error
      ".maya-field-wrap{position:relative;display:grid}.maya-field-wrap input{padding-right:56px}.maya-icon-button{position:absolute;right:8px;top:50%;transform:translateY(-50%);width:40px;height:40px;display:grid;place-items:center;border:1px solid #CDBA98;border-radius:10px;background:#EDE3CF;color:#1E2A42;font-size:18px;font-weight:700;cursor:pointer;transition:background .15s ease}.maya-icon-button:hover{background:#DCC9A3}.maya-hidden-picker{position:absolute;right:8px;top:50%;width:40px;height:40px;opacity:0;pointer-events:none}.maya-field-hint{color:#948C7B;font-family:'IBM Plex Sans',sans-serif;font-size:12px;line-height:1.4}.maya-field-error{display:none;margin-top:8px;padding:9px 11px;background:#F7E8E4;color:#5A1E1E;border:1px solid rgba(90,30,30,.22);border-radius:10px;font-family:'IBM Plex Sans',sans-serif;font-size:13px;line-height:1.4}.maya-field-error.is-visible{display:block}",
      // Remedy report — Cinzel h4, left navy border accent
      ".maya-report{display:grid;gap:14px}.maya-remedy{padding:15px;background:#F7F1E4;border:1px solid #CDBA98;border-left:4px solid #34456B;border-radius:12px;box-shadow:0 8px 24px rgba(30,42,66,.08)}.maya-remedy h4{margin:0 0 8px;color:#1E2A42;font-family:'Cinzel',serif;font-weight:600;font-size:17px}.maya-remedy p{margin:0 0 9px;color:#6B6357;font-family:'IBM Plex Sans',sans-serif;font-size:13px;line-height:1.5}.maya-remedy a{display:inline-flex;align-items:center;justify-content:center;min-height:40px;padding:0 16px;background:#1E2A42;color:#EDE3CF;border-radius:999px;text-decoration:none;font-family:'IBM Plex Sans',sans-serif;font-weight:600;font-size:13px}.maya-disclaimer{padding:12px;background:#DCC9A3;color:#6B6357;border-radius:10px;font-family:'IBM Plex Sans',sans-serif;font-size:12px;line-height:1.5}.maya-whatsapp{display:inline-flex;align-items:center;justify-content:center;min-height:46px;padding:0 18px;background:#176f46;color:#fffaf0;border-radius:999px;text-decoration:none;font-family:'IBM Plex Sans',sans-serif;font-weight:700}",
      // Step animation (unchanged keyframe, same name)
      ...
      // Progress bar transition (unchanged)
      ".maya-progress span{transition:width .45s cubic-bezier(.22,.61,.36,1)}",
      // Choice card hover — softer navy shadow
      ".maya-choice-card{transition:transform .18s ease,box-shadow .18s ease,border-color .18s ease}.maya-choice-card:hover{transform:translateY(-2px);box-shadow:0 16px 38px rgba(30,42,66,.14);border-color:#A98F66}.maya-choice-card:active{transform:translateY(0)}",
      // Goal selection cards — single column, navy selected state
      ".maya-choice-grid--goal{grid-template-columns:1fr}.maya-choice-card--goal{min-height:0;padding:15px 17px;display:flex;align-items:center;gap:14px;position:relative}.maya-choice-card--goal .maya-goal-icon{width:38px;height:38px;flex-shrink:0;border-radius:999px;display:grid;place-items:center;font-size:18px;background:#DCC9A3;border:1px solid #A98F66;color:#1E2A42}.maya-choice-card--goal .maya-goal-text{display:grid;gap:2px}.maya-choice-card--goal strong{font-family:'Cinzel',serif;font-weight:600;font-size:15px;line-height:1.2;color:#1E2A42}.maya-choice-card--goal span{font-family:'IBM Plex Sans',sans-serif;font-size:12.5px;color:#6B6357}.maya-choice-card--goal.is-selected{border-color:#34456B;background:#DCC9A3;box-shadow:0 0 0 2px rgba(52,69,107,.24),0 8px 24px rgba(30,42,66,.12)}.maya-choice-card--goal.is-selected::after{content:'\\2713';position:absolute;right:14px;top:50%;transform:translateY(-50%);width:24px;height:24px;border-radius:999px;background:#34456B;color:#EDE3CF;display:grid;place-items:center;font-size:13px;font-weight:900}",
      // City autocomplete dropdown
      ".maya-city-wrap{position:relative}.maya-city-list{position:absolute;left:0;right:0;top:calc(100% + 6px);z-index:5;background:#F7F1E4;border:1px solid #CDBA98;border-radius:12px;box-shadow:0 16px 44px rgba(30,42,66,.18);max-height:260px;overflow:auto;display:none}.maya-city-list.is-open{display:block}.maya-city-option{display:flex;flex-direction:column;gap:1px;width:100%;text-align:left;padding:11px 14px;border:0;border-bottom:1px solid #CDBA98;background:transparent;cursor:pointer;font-family:inherit}.maya-city-option:last-child{border-bottom:0}.maya-city-option:hover,.maya-city-option.is-active{background:#DCC9A3}.maya-city-option strong{font-family:'IBM Plex Sans',sans-serif;font-size:14px;color:#1E2A42;font-weight:700}.maya-city-option small{font-family:'IBM Plex Sans',sans-serif;font-size:12px;color:#6B6357}.maya-city-confirmed{display:flex;align-items:center;gap:8px;margin-top:8px;padding:8px 12px;background:#E8F0E0;border:1px solid rgba(39,80,10,.2);border-radius:10px;color:#1E3410;font-family:'IBM Plex Sans',sans-serif;font-size:12.5px;font-weight:700}.maya-city-unconfirmed{display:flex;align-items:center;gap:8px;margin-top:8px;padding:8px 12px;background:#F7E8E4;border:1px solid rgba(90,30,30,.2);border-radius:10px;color:#5A1E1E;font-family:'IBM Plex Sans',sans-serif;font-size:12.5px;line-height:1.45}",
      // Responsive breakpoints (unchanged logic, updated border-radius on mobile)
  ...
  function createWidget() {
    ...
    root.innerHTML = [
      '<button class="maya-launcher" type="button" data-maya-open>&#10022; Ask Maya</button>',
      ...
      var options = {
        ...
        handler: async function (response) {
          try {
            ...
            addCredits(Number(verified.credits || 300));
            renderCreditBar();
            addMessage("Beautiful. Maya has added 300 credits to your balance. We can go deeper now.", "bot");
          } catch (error) {
    ...
    try {
      ...
      if (data.stage === "ready") {
        ...
        if (typeof data.free_remaining === "number") {
          state.credits = data.free_remaining + Number(data.paid_credits || 0);
          saveCredits();
        }
  ...
  async function submitAstroSetup() {
    ...
    try {
      ...
      state.astroStage = "ready";
      if (typeof data.free_remaining === "number") {
        state.credits = data.free_remaining;
        saveCredits();
      }
      document.getElementById("mayaChatFoot").style.display = "flex";
      document.getElementById("mayaBody").innerHTML = creditBarHtml();
  ...
  async function sendChat(text) {
    ...
    try {
      ...
      var data = await postJson(["/api/message"], { session_id: state.sessionId, message: text });
      var usedAmount = null;
      if (typeof data.free_remaining === "number") {
        usedAmount = Math.max(0, previousCredits - data.free_remaining);
        state.credits = data.free_remaining;
        saveCredits();
      } else {
        usedAmount = Math.min(state.credits, nextCreditUse());
        deductCredits(usedAmount);
      }
      renderCreditBar();
      ...
      if (usedAmount && !data.locked) addMessage("Maya used " + usedAmount + " credits for this reading. Balance: " + creditsText() + ".", "bot");
      if (data.locked) {
        showUpgradePrompt();
